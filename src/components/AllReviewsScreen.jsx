...
import { FaMusic, FaHeart, FaSearch, FaUser, FaStar, FaRegStar, FaArrowLeft, FaComment, FaRegHeart, FaGlobe } from 'react-icons/fa';
...
const AllReviewsScreen = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      musicData.forEach(music => {
        const musicDummyReviews = generateDummyReviews(music, music.id);
        dummyReviews = [...dummyReviews, ...musicDummyReviews];
      });
  ...
  const generateDummyReviews = (music, musicId) => {
    ...
    // 더미 리뷰 생성
    const dummyReviews = [];
    const reviewCount = musicId % 2 === 0 ? 1 : 2; // 짝수면 1개, 홀수면 2개 리뷰
    ...
    for (let i = 0; i < reviewCount; i++) {
      ...
      // 내용 선택
      const contentIndex = Math.floor(Math.random() * reviewTexts.length);
      const content = reviewTexts[contentIndex];
  ...
  const navigateToMusicDetail = (musicId) => {
  ...
  const handleWriteReview = (musicId) => {
    navigate(`/review/${musicId}`);
  };
  ...
  return (
    <div className="screen-container">
      <div className="phone-frame">
        ...
        {loading ? (
          ...
          <div className="reviews-list">
            {reviews.length > 0 ? (
              reviews.map(review => {
                const music = allMusic.find(m => m.id === review.musicId);
                return (
                  <div key={review.id} className="review-item">
                    <div className="review-music-info" onClick={() => navigateToMusicDetail(review.musicId)}>
                      <div className="thumbnail">{music?.thumbnail || '🎵'}</div>
                      <div className="item-content">
                        <div className="item-title" title={music?.title || '알 수 없는 음악'}>{music?.title || '알 수 없는 음악'}</div>
                        <div className="item-artist" title={music?.artist || '알 수 없는 아티스트'}>{music?.artist || '알 수 없는 아티스트'}</div>
                      </div>
                    </div>
                    ...
                    <div className="reviewer-info">
                      ...
                      <div className="reviewer-details">
                        <div className="reviewer-name">
                          {review.username} 
                          {review.userId === username && <span> (나)</span>}
                        </div>
              ...
              <div className="empty-reviews">아직 등록된 리뷰가 없습니다.</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="tab-bar">
        ...
        <div className="tab active" onClick={() => handleNavigation('/allreviews')}>
          <FaGlobe />
        </div>
